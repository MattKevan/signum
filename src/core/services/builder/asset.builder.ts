...
import { LocalSiteData, SiteBundle, ImageRef } from '@/core/types';
import { getAssetContent, getJsonAsset, ThemeManifest, LayoutManifest } from '@/core/services/config/configHelpers.service';
...
async function bundleAssetFiles(
    ...
    await Promise.all(manifest.files.map(async (file) => {
        ...
        if (content) {
            const bundlePath = `${assetType}s/${assetId}/${file.path}`;
